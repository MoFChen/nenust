...
下面区分 Typst 中需要特殊处理、目前仍无法完整处理以及已经支持但容易被误述的排版能力。部分问题及其解决方法引用自：
...
*模板现状*：A3 封面的书脊由 `template/pages/cover.typ` 调用 `template/modules/utils.typ` 中的 `v_cjk_latin`，逐个排列字符并将拉丁字母旋转 90 度。这只是书脊专用模拟，不支持正文直排；标题含较多拉丁字符时差异尤其明显。
...
*模板现状*：模板使用多处 `show` 规则设置样式，普通字重的中文标点不会被全局替换；但伪粗体规则会重绘粗体中文及部分相邻标点，粗体连续标点仍可能触发这一问题。用户添加其他标点美化规则或引入会改写标点的包时也需要专门检查。
...
*模板现状*：`template/config/fonts.typ` 使用 `covers: "latin-in-cjk"` 区分中西文字体；`template/nenu-template.typ` 尝试让 `smartquote` 使用同一个 `FONT_TIMES`，但该字体设置带有覆盖范围限制，转换后的引号仍可能使用回退字体，不能保证一定来自 Times New Roman。

*实际处理*：中文正文使用输入法直接输入成对的 `“”`、`‘’`，让它们采用中文字体；普通西文引号可输入 ASCII `'`、`"`，交由 `smartquote` 转换。词首撇号等容易被误判的情况（如 `’t Hooft`）应直接输入右撇号，并用不带覆盖限制的西文字体局部包裹。复杂嵌套若转换错误，也应直接输入最终弯引号并检查字体，不要继续叠加自动替换规则。
...
Typst 有原生 `footnote`，模板还通过正文开始前设置的页眉上下文，让正文及其后页面的脚注圈码逐页重新编号；更早的摘要、目录等前置内容不在这条重置规则内。clreq-gap 的“脚注、尾注等”章节目前只是待继续调查，不能据此推断中文脚注不可用。标题、图题或表题中的脚注可能在相应目录中再次排版；其中的 `footcite` 还会重复参与文献排序和回链收集。最简单的做法是只在普通正文中使用脚注和文献脚注，确有需要时再采用状态变量变通。
