...
Typst 有原生 `footnote`，模板还通过正文开始前设置的页眉上下文，让正文及其后页面的脚注圈码逐页重新编号；更早的摘要、目录等前置内容不在这条重置规则内。clreq-gap 的“脚注、尾注等”章节目前只是待继续调查，不能据此推断中文脚注不可用。已知的具体边界之一是标题中的脚注也会出现在目录内容中；最简单的做法是避免在标题中使用脚注，确有需要时再采用状态变量变通。
