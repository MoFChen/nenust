...
但 Typst 是支持斜体的；仓库附带的 Roboto Mono 覆盖斜体字形，可得到斜体样式 #text(font: "Roboto Mono")[_ABC123_]。
...
脚注代码 `#footnote[补充说明]`。模板为正文及其后的页面设置页眉，并在该作用域内逐页重置脚注编号；摘要、目录等更早的前置内容不在这条重置规则内。不要手工输入圈码或在正文中模拟脚注线#footnote[这是一个脚注]。脚注内容为文献引用时使用 `footcite`，具体写法见@sec-footcite。
